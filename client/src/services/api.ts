...
export const batchAPI = {
  ...
  createBatch: async (data: CollectionFormData): Promise<ApiResponse<Batch>> => {
  ...
  updateBatchStatus: async (id: string, status: string, notes?: string): Promise<ApiResponse<Batch>> => {
    const response = await api.patch(`/batches/${id}/status`, { status, notes });
    return response.data;
  },
...
export const collectorAPI = {
  ...
  updateProfile: async (id: string, data: Partial<Collector>): Promise<ApiResponse<Collector>> => {
    const response = await api.patch(`/collectors/${id}`, data);
    return response.data;
  },

  // Get collector's batches
  getBatches: async (id: string, params?: { page?: number; limit?: number }): Promise<PaginatedResponse<Batch>> => {
...
export const labAPI = {
  ...
  uploadTestResults: async (data: LabTestFormData): Promise<ApiResponse<LabResult>> => {
    const formData = new FormData();
  ...
  getBatchResults: async (batchId: string): Promise<ApiResponse<LabResult[]>> => {
...
export const ipfsAPI = {
  // Upload file to IPFS
  uploadFile: async (file: File): Promise<ApiResponse<{ hash: string; url: string }>> => {
    const formData = new FormData();
  ...
  uploadJSON: async (data: any): Promise<ApiResponse<{ hash: string; url: string }>> => {
...
export const blockchainAPI = {
  ...
  verifyBatch: async (batchId: string): Promise<ApiResponse<{ verified: boolean; txHash: string }>> => {
...
export const qrAPI = {
  ...
  generateQR: async (batchId: string): Promise<ApiResponse<{ qrCode: string; url: string }>> => {
    const response = await api.post('/qr/generate', { batchId });
    return response.data;
  },

  // Validate QR code
  validateQR: async (qrCode: string): Promise<ApiResponse<{ valid: boolean; batchId?: string }>> => {
